    ...
    try {
      ...
      if (item.type === "license_issued") {
        ...
        await resend.emails.send({
          ...
          text: [
            `Đăng nhập vào ứng dụng Vô chi bằng chính email này (${item.recipient}) để kích hoạt gói ${payload.plan} ngay — không cần nhập gì thêm.`,
            "Tải app tại: https://vochi.xyz/download",
            "",
            "1. Mở app Vô chi",
            "2. Đăng nhập (hoặc đăng ký) bằng email này",
            "3. Pro tự động kích hoạt",
            "",
            "License key của bạn (dùng khi tặng quà, dùng offline, hoặc muốn kích hoạt bằng email khác):",
            "",
            `Plan: ${payload.plan}`,
        ...
        await resend.emails.send({
          ...
          text: [
            "Mỗi người nhận key có thể đăng nhập bằng email họ dùng khi bạn kích hoạt hộ, hoặc dán key trực tiếp vào app.",
            "",
