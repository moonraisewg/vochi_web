import { oauthLogin } from "@/lib/server/auth";
import { googleOAuthSchema } from "@/lib/server/authPolicy";
...
export async function POST(req: Request) {
  try {
    const input = googleOAuthSchema.parse(await req.json());
    await assertAuthThrottle(`auth:oauth:ip:${clientIp(req)}`, 30, 60 * 60 * 1000);
